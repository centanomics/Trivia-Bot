...
export default class extends Command {
  ...
  async run(message) {
    ...
    try {
      ...
      triviaData = await (
        await axios(`https://opentdb.com/api.php?amount=10&type=multiple`)
      ).data.results;
    } catch (e) {
    ...
    for (let i = 0; i < triviaData.length; i++) {
      ...
      const filter = (reaction, user) => {
        ...
        return (
          (reaction.emoji.name === answer || reaction.emoji.name === '🛑') &&
      ...
      const collector = msgEmbed.createReactionCollector(filter, {
        time: 10000,
      }); // will only collect for 10 seconds
          ...
          let result = newEmbed
            ...
            .setDescription(
              '\n The correct answer was ' +
                parseEntities(triviaData[i].correct_answer)
            );
          //   Util.log(`Correct answer: ${triviaData[i].correct_answer}`, false);
    ...
    if (counter === 0) {
      ...
      if (Object.keys(leaderboard).length !== 0) {
        ...
        winnerEmbed
          .setTitle('Game Over! No one got anything right...')
          .setColor('#5fdbe3');
        // send the embed to the channel
